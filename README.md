...
Netlify へのデプロイを想定した React + Vite（TypeScript）の UI プロトタイプを `frontend/` に配置しています。問い合わせ詳細・参照ナレッジと AI ドラフトを並列で表示し、「送信承認」ボタンや自動送信トグルを備えたレビュー体験を確認できます。ヘッダーの `Admin` リンクからは管理コンソール（Gemini 設定・プロンプト編集・ユーザー管理）へ遷移できます。
...
Admin コンソールでは、Gemini API キーと自動送信しきい値、システム／フォールバックプロンプト、ユーザーアカウント情報（氏名・メール・電話・住所・役割）の編集と、ユーザーデータ保存先スプレッドシートの設定が行えます。
